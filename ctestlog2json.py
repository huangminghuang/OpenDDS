...
class PreOutputSectionProessor(object):
    ...
    def testing_matched(self, match):
        self.test_info = { 'number': match.group(1), 'name': match.group(2).replace('__', ' '), 'output': [], 'errors': []}
...
class OutputSectionProcessor(object):
    ...
    def process(self, line):
        if self.end_of_output:
            ...
            if "ERROR" in line:
              self.test_info['errors'].append(line)
    def next(self):
...
def get_repo_info(dir):
    return{
        'origin': os.getenv('GIT_ORIGIN') or subprocess.check_output(["git", "remote", "get-url", "origin"], cwd=dir),
...
def parse_args():
  ...
  parser.add_argument('--matrix',
                      help='Specifify the matrix name.')
...
def main():
   ...
   if not config.lasttestlog:
     ...
     result['matrix'] = config.matrix

     result['build_flags'] = os.getenv('DDS_BUILD_FLAGS') or ""

     fn = "{}.json".format(result['repo']['commit'])
   else:
     fn = "tests.json"

   with open(os.path.join(config.output_dir, fn), 'w') as f:
       json.dump(result, f)
